...
export default function AdminAuditPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setIsLoading(true);
        const q = query(adminAuditCollection, orderBy("createdAt", "desc"), limit(200));
        const snap = await getDocs(q);
        const list: AuditDoc[] = [];
        snap.forEach((d) => list.push({ id: d.id, ...d.data() } as AuditDoc));
        setEntries(list);
      } catch (err) {
    ...
    <section className="page-section">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-balance text-fluid-subtitle text-muted-foreground">
          Historial de cambios administrativos realizados en el sistema.
        </p>
      </header>
